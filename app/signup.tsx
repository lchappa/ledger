            ...
            <View style={styles.container}>
                ...
                <View style={styles.inputView}>
                    <TextInput style={styles.input} placeholder='EMAIL' value={email} onChangeText={setEmail} autoCorrect={false}
                               autoCapitalize='none' />
                    <TextInput style={styles.input} placeholder='PASSWORD' onChangeText={setPassword} secureTextEntry autoCorrect={false}
                               autoCapitalize='none'/>
                </View>
...
function AccountCreator(email: string, password: string) {
    auth().createUserWithEmailAndPassword(email, password)
        .then(() => {
            ...
            if (userid){
                firestore().collection('users').doc(auth().currentUser?.uid).set({total_amount: 0.00, spot_amount: 0.00, earn_amount: 0.00}).then();
            }
